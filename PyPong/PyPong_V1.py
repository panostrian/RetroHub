...
import time
import os
os.chdir("C:\My_Coding_Projects\Python_Projects\PyPong")
...
def update_ball():
    ...
    if ball.colliderect(player1_paddle):
        if ball.right > player1_paddle.left and ball_speed_x < 0:
            # Ball hit player 1's paddle from the right side
            ball_speed_x *= -1
            bounce_sound.play()
        elif ball.left < player1_paddle.right and ball_speed_x > 0:
            # Ball hit player 1's paddle from the left side
            ball_speed_x *= -1
            bounce_sound.play()
        else:
            # Ball hit player 1's paddle from the top or bottom
            ball_speed_y *= -1
            bounce_sound.play()

    if ball.colliderect(player2_paddle):
        if ball.left < player2_paddle.right and ball_speed_x > 0:
            # Ball hit player 2's paddle from the left side
            ball_speed_x *= -1
            bounce_sound.play()
        elif ball.right > player2_paddle.left and ball_speed_x < 0:
            # Ball hit player 2's paddle from the right side
            ball_speed_x *= -1
            bounce_sound.play()
        else:
            # Ball hit player 2's paddle from the top or bottom
            ball_speed_y *= -1
            bounce_sound.play()
    # update ball position
    ball.x += ball_speed_x
